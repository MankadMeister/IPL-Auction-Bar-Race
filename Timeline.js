...
var Bar = function (i, assigned_width, name, entity) {
  ...
  if (shouldAssignIdentity) {
    pickedCol = mapping[this.name].color;
  } else {
    pickedCol = randomColors?.[i] || [0, 0, 0];
  }
  // console.log(this.name.includes('KKR'));
  if (this.name.includes('KKR')) {
    this.r = 148;
    this.g = 0;
    this.b = 211;
  } else if (this.name.includes('CSK')) {
    this.r = 255;
    this.g = 215;
    this.b = 10;
  }
  else if (this.name.includes('DD')) {
    this.r = 255;
    this.g = 0;
    this.b = 0;
  }
  else if (this.name.includes('RCB')) {
    this.r = 128;
    this.g = 0;
    this.b = 0;
  }
  else if (this.name.includes('MI')) {
    this.r = 135;
    this.g = 206;
    this.b = 250;
  }
  else if (this.name.includes('Deccan DC')) {
    this.r = 0;
    this.g = 0;
    this.b = 128;
  }
  else if (this.name.includes('RR')) {
    this.r = 100;
    this.g = 139;
    this.b = 247;
  }
  else if (this.name.includes('KXIP')) {
    this.r = 240;
    this.g = 128;
    this.b = 128;
  }
  else if (this.name.includes('KTK')) {
    this.r = 255;
    this.g = 140;
    this.b = 0;
  }
  else if (this.name.includes('PWI')) {
    this.r = 0;
    this.g = 0;
    this.b = 0;
  }
  else if (this.name.includes('PBKS')) {
    this.r = 255;
    this.g = 69;
    this.b = 0;
  }
  else if (this.name.includes('DC')) {
    this.r = 0;
    this.g = 0;
    this.b = 255;
  }
  else if (this.name.includes('RPS')) {
    this.r = 199;
    this.g = 21;
    this.b = 133;
  }
  else if (this.name.includes('GL')) {
    this.r = 255;
    this.g = 165;
    this.b = 0;
  }
  else if (this.name.includes('GT')) {
    this.r = 25;
    this.g = 25;
    this.b = 112;
  }
  else if (this.name.includes('LSG')) {
    this.r = 176;
    this.g = 224;
    this.b = 330;
  }
  else if (this.name.includes('SRH')) {
    this.r = 255;
    this.g = 165;
    this.b = 0;
  }
